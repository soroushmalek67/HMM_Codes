function [ts_full, fr, dur, epochs, fr_n, bin, hmm] = compute_hmm_output(job, dataset, sleep, threshold, out_dir, plot_figs, num_states)
    ...
    while ~strcmp(job.State, 'finished') && ~strcmp(job.State, 'failed')
        pause(5)
    end
    [hmm, emis, trans, l] = deal(job.Tasks(1).OutputArguments{:});
    save(hmm_file, 'hmm', 'emis', 'trans', 'l');
